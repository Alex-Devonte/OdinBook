    ...
    return (
      <div className="bg-slate-200 md:mr-8">
        <Tabs defaultValue={1}>
          ...
          {followers &&
            followers.filter((follower) => follower.status === "pending")
              .length === 0 && (
              <TabPanel value={1}>
            ...
            <ul className="flex flex-row gap-10 justify-start items-stretch overflow-x-scroll text-center lg:flex-col lg:items-start lg:justify-between lg:overflow-x-auto lg:gap-2">
              {followers &&
                followers.map((follower, i) => (
                  ...
                  <TabPanel
                    key={i}
                    value={follower.status === "pending" ? 1 : 2}
                    className="lg:w-full"
                  >
                    <li
                      key={i}
                      className={`flex flex-col justify-between p-3 gap-5 h-full lg:flex-row lg:items-stretch lg:w-full hover:bg-slate-50 ${
                    ...
                    >
                      <div className="flex flex-col items-center lg:flex-row lg:gap-5 lg:w-full">
                        <img
                          ...
                          src={follower.user?.profilePicture}
                        />
                        <p>
                      ...
                      {follower.status === "pending" && (
                        <div className="flex flex-col items-center justify-end gap-5 mt-5 lg:flex-row lg:mt-0 lg:p-3 ">
                          <button
                            onClick={(e) => respondToRequest(e, follower.user)}
                            value="accepted"
                            className="bg-slate-700 text-white p-1 shadow-md rounded-xl w-20 transition duration-100 ease-linear hover:bg-odin-gold active:bg-odin-gold"
                          >
                            Accept
                          </button>
                          <button
